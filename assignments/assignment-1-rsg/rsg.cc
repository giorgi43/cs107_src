...
/*
*  Takes grammar and starting point (seed) to generate random text 
*  with specified grammar rules
*  @param grammar STL map reference, which maps nonterminal strings to their definitions
*  @param seed reference to Definition, starting point. (normally <start>)
*/
void generate(const map<string, Definition>& grammar, const Definition& seed) {
	const Production randProd = seed.getRandomProduction();
	for (const auto& u : randProd) {
		if (u[0] != '<') {
			cout << u << ' ';
		} else {
			const Definition& newSeed = grammar.at(u);
			generate(grammar, newSeed);
		}
	}
}
...
int main(int argc, char *argv[])
{
  ...
  cout << "Version #1: ---------------------------\n\t";
  generate(grammar, grammar["<start>"]);
  cout << "\nVersion #2: ---------------------------\n\t";
  generate(grammar, grammar["<start>"]);
  cout << "\nVersion #3: ---------------------------\n\t";
  generate(grammar, grammar["<start>"]);
